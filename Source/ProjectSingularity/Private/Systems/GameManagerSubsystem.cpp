...
void UGameManagerSubsystem::SetGameState(EGameState _eNewState) {
  ...
  if (!CanTransition(m_eCurrentGameState, _eNewState)) {
    UE_LOG(LogTemp, Warning, TEXT("[GAME_STATE] Invalid GameState transition: %d -> %d"), (uint8)m_eCurrentGameState, (uint8)_eNewState);
    return;
  }
...
void UGameManagerSubsystem::Initialize(FSubsystemCollectionBase& _rCollection) {
  ...
  m_mValidTransitions.Add(EGameState::PLAYING, {
      EGameState::PAUSED,
      EGameState::GAMEOVER
    }
  );
